...
「今回のデモでは、Symbolブロックチェーンを活用した学習証跡管理システムを紹介します。このシステムは、学習活動を改ざん不可能な証跡として記録し、就職活動での信頼性の高いポートフォリオとして活用できます。」

#### 2. 学習記録の作成（2分）

1. ホームページを開く
2. 学習記録フォームに入力:
   - タイトル: "Next.js App Routerの学習"
   - 科目: "Webフレームワーク"
   - 学習時間: 90分
   - 理解度: 4
   - メモ: "サーバーコンポーネントとクライアントコンポーネントの使い分けを理解"
3. 「保存」ボタンをクリック
4. **ポイント説明**: 「この時点では、まだブラウザ内のLocalStorageに保存されているだけです」

#### 3. ブロックチェーン登録（2分）

1. 「保留中」ページに移動
2. 作成したレコードを確認
3. 「ブロックチェーンに登録」ボタンをクリック
4. SSS Extensionのポップアップで内容を確認:
   - 「メッセージには学習記録がJSON形式で含まれています」
   - 「自分自身へのTransferTransactionです」
5. 「承認」をクリック
6. 待機中（~30秒）: 「Symbolブロックチェーンのブロック承認を待っています」
7. 承認完了後、「確定済み」ページに移動
8. **ポイント説明**: 「トランザクションハッシュとブロック高が表示されています。これがブロックチェーン上の証明です」
...
4. 検証結果を説明:
   ...
   - ✅ メッセージがデコードされました
   - ✅ ブロックで確認されました
5. 「Symbol Explorerで確認」リンクをクリック
6. Symbol Explorer上でトランザクション詳細を表示
7. **ポイント説明**: 「第三者がいつでもこのトランザクションハッシュで記録の真正性を検証できます。ブロックチェーンに記録されているため、後から改ざんすることは不可能です」
...
2. 確定済みレコードが階層構造で表示される
3. ズーム・パン機能をデモ
4. **ポイント説明**: 「学習の進捗を視覚的に把握できます」

#### 6. ポートフォリオ共有（30秒）

1. 「ポートフォリオ」ページに移動
2. 統計情報を表示
3. **ポイント説明**: 「このURLを企業に共有することで、客観的な学習履歴を証明できます」

#### 7. クロージング（30秒）

「まとめると、このシステムは以下の特徴があります：
- ✅ ブロックチェーンで改ざん不可能な記録
- ✅ 第三者が即座に検証可能
- ✅ 就職活動での信頼性の高いポートフォリオ
- ✅ 今後はブラウザ拡張での自動記録も可能
...
- **Symbol Community**: ブロックチェーン技術の提供とサポート
- **Next.js Team**: 優れたReactフレームワーク
- **React Flow**: 学習ツリー可視化ライブラリ
